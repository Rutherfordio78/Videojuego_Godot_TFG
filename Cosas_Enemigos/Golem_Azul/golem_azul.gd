...
var rango_melee: float = 60.0          # Rango para iniciar el ataque melee
	...
	if jugador:
		...
		if distancia_al_jugador <= rango_persecucion and abs(jugador_offset) <= patrol_range:
			...
			if distancia_al_jugador <= rango_melee and timer_ataque.is_stopped():
				...
				timer_ataque.start(1.0)  # Ajusta el cooldown de ataque según tus necesidades.
			else:
				mover_hacia(jugador.position)
...
func cambiar_estado(nuevo_estado: Estado) -> void:
	...
	match nuevo_estado:
		...
		Estado.ATAQUE_MELEE:
			...
			velocity.x = 0
			if direccion_num == 1:
				animation_player.play("Ataque_Derecha")
			else:
				animation_player.play("Ataque_Izquierda")
		Estado.HERIDO:
			herido = true
			animation_player.play("Herido")
...
func _on_eliminar_enemigo() -> void:
	var dropped_item = Global.Dropear_Items_Especiales()[0]
