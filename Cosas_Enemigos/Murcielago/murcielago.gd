...
var rango_perseguir: float = 200.0
...
var estado_actual: Estado = Estado.DORMIDO
...
@onready var cooldown_ataque: Timer = $Cooldown_Ataque
@onready var volar: AudioStreamPlayer2D = $volar
...
func _physics_process(delta: float) -> void:
	...
	match estado_actual:
		Estado.DORMIDO:
			process_dormido(delta)
		Estado.DESPERTARSE:
			process_despertarse(delta)
		Estado.IDLE_VOLAR:
...
# Estado DORMIDO: inactivo hasta que el jugador se acerca
func process_dormido(delta: float) -> void:
	var player_pos = get_player_position()
	if global_position.distance_to(player_pos) < distancia_despertar:
		estado_actual = Estado.DESPERTARSE
		animation_player.play("Despertarse")

# Estado DESPERTARSE: espera a que termine la animación para pasar a IDLE_VOLAR
func process_despertarse(delta: float) -> void:
	# Se transiciona en _on_animation_finished
	pass
...
func process_perseguir(delta: float) -> void:
	animation_player.play("Volar_Rapido")
	var player_pos = get_player_position()
	...
	if dist_to_player < rango_ataque and cooldown_ataque.is_stopped():
		estado_actual = Estado.ATAQUE
		# Seleccionamos animación de ataque según la posición relativa
		if player_pos.x < global_position.x:
			animation_player.play("Ataque_1_Izquierda")
		else:
			animation_player.play("Ataque_1_Derecha")
	...
	# Si el jugador se aleja, volvemos a IDLE_VOLAR
		...
		animation_player.play("Idle_Volar")
...
func recibir_daño(daño_recibido: int) -> void:
	...
	print("Murciélago recibió daño: ", daño_recibido, " | Vida actual:", vida_enemigo)
...
func _on_animation_finished(anim_name: String) -> void:
	# Al terminar la animación de despertar, pasar a IDLE_VOLAR
	if estado_actual == Estado.DESPERTARSE:
		estado_actual = Estado.IDLE_VOLAR
		animation_player.play("Idle_Volar")
